<?php
	...
	function sitemap_build($action, $settings, $board) {
		...
		$action = generation_strategy('sb_sitemap', []);
		...
		elseif ($action == 'rebuild') {
			...
			file_write($settings['path'], Element('themes/sitemap/sitemap.xml', ['settings' => $settings, 'config' => $config, 'threads' => $threads, 'boards' => $boards]));
		}
	}
