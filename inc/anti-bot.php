...
class AntiBot {
	...
	public static function randomString($length, $uppercase = false, $special_chars = false, $unicode_chars = false) {
		...
		if ($unicode_chars) {
			$len = strlen($chars) / 10;
			for ($n = 0; $n < $len; $n++)
				$chars .= mb_convert_encoding('&#' . random_int(0x2600, 0x26FF) . ';', 'UTF-8', 'HTML-ENTITIES');
		}
		...
		while (count($ch) < $length) {
			$n = $length - count($ch);
			$keys = array_rand($chars, $n > (is_countable($chars) ? count($chars) : 0) ? count($chars) : $n);
			if ($n == 1) {
				$ch[] = $chars[$keys];
				break;
			}
	...
	public static function make_confusing($string) {
		...
		foreach ($chars as &$c) {
			if (random_int(0, 3) != 0)
				...
				$c = mb_encode_numericentity((string) $c, [0, 0xffff, 0, 0xffff], 'UTF-8');
		}
	...
	public function __construct(array $salt = []) {
		...
		$input_count = random_int($config['spam']['hidden_inputs_min'], $config['spam']['hidden_inputs_max']);
		$hidden_input_names_x = 0;
		
		for ($x = 0; $x < $input_count ; $x++) {
			if ($hidden_input_names_x === false || random_int(0, 2) == 0) {
				// Use an obscure name
				$name = static::randomString(random_int(10, 40), false, false, $config['spam']['unicode']);
			} else {
				// Use a pre-defined confusing name
				$name = $config['spam']['hidden_input_names'][$hidden_input_names_x++];
				if ($hidden_input_names_x >= (is_countable($config['spam']['hidden_input_names']) ? count($config['spam']['hidden_input_names']) : 0))
			...
			if (random_int(0, 2) == 0) {
				...
				// Numeric value
				$this->inputs[$name] = (string)random_int(0, 100000);
			} else {
				// Obscure value
				$this->inputs[$name] = static::randomString(random_int(5, 100), true, true, $config['spam']['unicode']);
			}
		}
	}
	...
	public static function space() {
		if (random_int(0, 3) != 0)
			return ' ';
		return str_repeat(' ', random_int(1, 3));
	}
	
	public function html($count = false) {
		global $config;
		
		$elements = ['<input type="hidden" name="%name%" value="%value%">', '<input type="hidden" value="%value%" name="%name%">', '<input name="%name%" value="%value%" type="hidden">', '<input value="%value%" name="%name%" type="hidden">', '<input style="display:none" type="text" name="%name%" value="%value%">', '<input style="display:none" type="text" value="%value%" name="%name%">', '<span style="display:none"><input type="text" name="%name%" value="%value%"></span>', '<div style="display:none"><input type="text" name="%name%" value="%value%"></div>', '<div style="display:none"><input type="text" name="%name%" value="%value%"></div>', '<textarea style="display:none" name="%name%">%value%</textarea>', '<textarea name="%name%" style="display:none">%value%</textarea>'];
		
		$html = '';
		
		if ($count === false) {
			$count = random_int(1, (int)abs((is_countable($this->inputs) ? count($this->inputs) : 0) / 15) + 1);
		}
		...
		foreach ($inputs as $name => $value) {
			...
			while (!$element) {
				...
				if (random_int(0, 5) == 0)
					$element = str_replace('>', self::space() . '>', $element);
				if (str_contains($element, 'textarea') && $value == '') {
			...
			if (random_int(0, 2) == 0)
				$value = static::make_confusing($value);
			else
				$value = utf8tohtml($value);
			
			if (!str_contains($element, 'textarea'))
